	...
	protected void paintComponent(Graphics g) {
		...
		for (int x = 0; x < this.nXCells; x++) {
			for (int y = 0; y < this.nYCells; y++) {
				...
				int neighbors = getCell((_x) % this.nXCells, (_y) % this.nYCells, this.renderedWord).alive
						+ getCell((x) % this.nXCells, (_y) % this.nYCells, this.renderedWord).alive +
						getCell((x_) % this.nXCells, (_y) % this.nYCells, this.renderedWord).alive
						+ getCell((x_) % this.nXCells, (y) % this.nYCells, this.renderedWord).alive +
						getCell((x_) % this.nXCells, (y_) % this.nYCells, this.renderedWord).alive
						+ getCell((x) % this.nXCells, (y_) % this.nYCells, this.renderedWord).alive +
						getCell((_x) % this.nXCells, (y_) % this.nYCells, this.renderedWord).alive
						+ getCell((_x) % this.nXCells, (y) % this.nYCells, this.renderedWord).alive;
...
public class HelpDialog {
	HelpDialog() {
		...
		Timer timer = new Timer(50, al);// create the timer which calls the actionperformed method for every 1000
